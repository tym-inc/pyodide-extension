...
import { OutputWebview } from "./outputWebview";
...
export class PyodideRunner {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this._outputWebview = new OutputWebview(this);
  }
  ...
  private async _setupWorker() {
    this._pyodideWorker = new Worker(this._workerScript);

    this._pyodideWorker.onmessage = (event) => {
      const { stdout, done, error, webgraphics } = event.data;

      if (webgraphics) {
        this._outputWebview.runGraphics(webgraphics.command, webgraphics.data);
